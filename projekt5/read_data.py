...
def game_state_to_data_sample_tail(game_state):
    ...
    head = game_state.get('snake_body')[-1]
    food_dict = {'food_up': 0, 'food_right': 0, 'food_down': 0, 'food_left': 0}

    if head[1] - food[1] > 0:
        food_dict.update({'food_up': 1})
    if head[0] - food[0] < 0:
        food_dict.update({'food_right': 1})
    if head[1] - food[1] < 0:
        food_dict.update({'food_down': 1})
    if head[0] - food[0] > 0:
        food_dict.update({'food_left': 1})

    collision_dict = {'collision_up': 0, 'collision_right': 0, 'collision_down': 0, 'collision_left': 0}

    if head[1] == 0:
        collision_dict.update({'collision_up': 1})
    if head[0] == 270:
        collision_dict.update({'collision_right': 1})
    if head[1] == 270:
        collision_dict.update({'collision_down': 1})
    if head[0] == 0:
        collision_dict.update({'collision_left': 1})
    ...
    for element in game_state["snake_body"][:-1]:
        body_values.append((element[0], element[1]))
    state1 = head[0] + 30
    state2 = head[0] - 30
    state3 = head[1] + 30
    ...
    if game_state["snake_direction"].value == 0: # up
        if (head[0], state4) in body_values:
            tail_forward = 1
        if (state1, head[1]) in body_values:
            tail_right = 1
    ...
    if game_state["snake_direction"].value == 1: # right
        if (state1, head[1]) in body_values:
            tail_forward = 1
        if (head[0], state3) in body_values:
            tail_right = 1
    ...
    if game_state["snake_direction"].value == 2: # down
        if (head[0], state3) in body_values:
            tail_forward = 1
        if (state1, head[1]) in body_values:
            tail_left = 1
    ...
    if game_state["snake_direction"].value == 3: # left
        if (state2, head[1]) in body_values:
            tail_forward = 1
        if (head[0], state3) in body_values:
            tail_left = 1
    ...
    row_list = list()
    row_list += list(food_dict.values())
    row_list += list(collision_dict.values())
    row_list += list( {'tail_forward': tail_forward, 'tail_right': tail_right, 'tail_left': tail_left}.values())
    list_of_data.append(row_list)
