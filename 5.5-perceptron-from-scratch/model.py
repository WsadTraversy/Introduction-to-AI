...
class Perceptron:
    ...
    def training(self, X, y, learning_rate=0.01):
        ...
        self._backward(X, y)
        for i, el in enumerate(self._weights):
            for j, layer in enumerate(el):
                layer -= learning_rate*self._dw[i][j]
        for el in self._bias:
            for j, bias in enumerate(el):
                bias -= learning_rate*self._db[i][j]
